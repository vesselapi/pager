...
const input = z.object({
 ...
 escalationPolicySteps: z.array(z.object({
    order: z.number(),
...
export const escalationPolicyCreate = trpc
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const escalationPolicy = await ctx.db.escalationPolicy.create({orgId,...input.escalationPolicy});
    const escalationPolicySteps = input.escalationPolicySteps.map(step => ({
        orgId,
        escalationPolicyId: escalationPolicy.id,
        ...step,
    }))
    const dbEscalationPolicySteps = await ctx.db.escalationPolicyStep.createMany(escalationPolicySteps)
